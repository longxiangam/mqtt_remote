...
sys.taskInit(function()
    ...
    wlan.init()
    -- 修改成自己的ssid和password
    wlan.connect("redmi_longxiang", "595744640+",1)
    -- wlan.connect("uiot", "")
...
function test_timer()
    set_timer_setting({{time="2025-02-15 12:05",enable=true,volume=3},{time="2025-02-15 12:06",enable=true,volume=3}})
end
...
function timer_trigger()
   local time_str = os.date("%Y-%m-%d %H:%M");
   local timer_setting = get_timer_setting();
